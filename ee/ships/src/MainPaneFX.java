...
public class MainPaneFX {
	...
	/**
	 * The MainPanel constructor sets up the GUI with buttons, radio buttons, and a
	 * display area.
	 */
	MainPaneFX(double CANVAS_WIDTH, double CANVAS_HEIGHT) {
		...
		addButton.setOnAction(event -> {
			try {
				...
				System.out
						.println("Name: " + name + ", Year Built: " + year + ", Type: " + type + ", " + tons + " Tons, "
								+ pax + " Passengers, " + ac + " Aircraft, " + guns + " Guns, " + torps + " Torpedoes");
		...
		/*
		 * When the read button is pushed, user is prompted to select an input file and
		 * ship data is read in.
		 */
		...
		// {ShipType.CARGO.toString(), ShipType.CRUISE.toString(),
		...
		// {ShipType.CARRIER.toString(), ShipType.CRUISER.toString(),
		// ShipType.DESTROYER.toString(), ShipType.MINE_SWEEPER.toString(),
		...
		// CARGO
		shipRadioButtons[0].setOnAction(event -> {
			clearFieldVisibility();
		...
		// CRUISE
		shipRadioButtons[1].setOnAction(event -> {
			clearFieldVisibility();
			inputLabels.getChildren().clear();
			inputPanel.getChildren().clear();
		...
		// WARSHIP
		shipRadioButtons[2].setOnAction(event -> {
		...
		// CARRIER
		warshipRadioButtons[0].setOnAction(event -> {
		...
		// CRUISER
		warshipRadioButtons[1].setOnAction(event -> {
		...
		// DESTROYER
		warshipRadioButtons[2].setOnAction(event -> {
		...
		// MINE_SWEEPER
		warshipRadioButtons[3].setOnAction(event -> {
		...
		// SUBMARINE
		warshipRadioButtons[4].setOnAction(event -> {
		...
		inputLabels.getChildren().addAll(nameLabel, yearLabel, shipLabel, gunsLabel, acLabel, torpLabel, pxLabel,
				tonsLabel);
		...
		inputPanel = new VBox();
		inputPanel.getChildren().addAll(nameField, yearField, shipField, gunsField, acField, torpField, pxField,
				tonsField);
	...
	private void writeFile() throws IOException {
		// Use JFileChooser instead of FileChooser so selected file can be a new one, as
		// well as an existing one
